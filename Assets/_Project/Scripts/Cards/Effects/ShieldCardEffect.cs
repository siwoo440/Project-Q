...
{
    [CreateAssetMenu(menuName = "Project Q/Cards/Effects/Shield Card Effect")] // 실드 카드 효과 에셋 메뉴 등록
    public sealed class ShieldCardEffect : CardEffect // 플레이어 실드 증가 카드 효과 클래스
    {
        [SerializeField] private float shieldAmount = 25f; // 카드 사용 시 추가 실드량

        public void ConfigureForEditor(float amount) // 에디터 자동 구성용 실드 수치 설정 메서드
        {
            shieldAmount = Mathf.Max(0f, amount); // 실드 증가량을 0 이상으로 보정
        }

        public override void Execute(CardEffectContext context) // 실드 카드 실제 효과 실행 메서드
        {
