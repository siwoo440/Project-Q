using ProjectQ.Combat; // 기존 아레나 설정 호환 기능 사용
...
namespace ProjectQ.Cards // 카드 시스템 네임스페이스
{
    public sealed class CardUseController : MonoBehaviour // 좌클릭·우클릭 2칸 카드 직접 사용 관리 클래스
    {
        ...
        [SerializeField] private ArenaController arena; // 기존 씬·에디터 설정 호환 아레나 참조
        ...
        public void Configure(RunDeck runDeck, PlayerStats stats, ArenaController arenaController) // 기존 전투 상태 포함 설정 호환 메서드
        {
            deck = runDeck; // 회차 덱 참조 저장
            playerStats = stats; // 플레이어 상태 참조 저장
            arena = arenaController; // 기존 아레나 참조 저장
        }

        private void Update() // 좌클릭·우클릭 카드 직접 사용 입력 처리 메서드
        {
            if (!CanUseCards() || Mouse.current == null) // 카드 사용 규칙과 마우스 입력 장치 사용 가능 여부 확인
            {
        ...
        {
            if (!CanUseCards() || deck == null || playerStats == null) // 카드 사용 규칙과 필수 참조 확인
            {
        ...
        {
            _ = arena; // 기존 직렬화 아레나 참조 호환 유지
            return true; // 전투 여부와 무관하게 카드 사용 항상 허용
        }
    }
}
