...
namespace ProjectQ.Player // 플레이어 시스템 네임스페이스
{
    public sealed class PlayerDebugController : MonoBehaviour // 플레이어 조작 상태 표시 클래스
    {
        ...
        private void OnGUI() // 플레이어 조작 정보 화면 표시 메서드
        {
            ...
            GUILayout.Label("Project Q - Day 4 Player Debug"); // 플레이어 디버그 제목 표시
            GUILayout.Label($"Move Input : {moveInput}"); // 현재 이동 입력 표시
            GUILayout.Label($"Velocity : {(movement != null ? movement.CurrentVelocity : Vector2.zero)}"); // 현재 실제 이동 속도 표시
            GUILayout.Label($"Aim Direction : {aimDirection}"); // 현재 자유 조준 방향 표시
            GUILayout.Label($"Dodge : {isDodging} / Invincible : {isInvincible}"); // 회피와 무적 상태 표시
            GUILayout.Label($"Dodge Cooldown : {cooldown:F2}s"); // 회피 남은 쿨타임 표시
            GUILayout.Label($"Hitbox Vulnerable : {canReceiveDamage}"); // 피격 가능 여부 표시
            GUILayout.Label("WASD / Left Stick + Mouse / Right Stick + Shift / South Button"); // 기본 조작 테스트 안내 표시
